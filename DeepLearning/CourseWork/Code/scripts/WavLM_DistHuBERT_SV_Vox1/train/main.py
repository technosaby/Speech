...
if __name__ == '__main__':
    ...
    # Check GPU environment
    if args['usable_gpu'] is None: 
        # On macOS, you might not have CUDA_VISIBLE_DEVICES; instead, check if a GPU is available
        if platform.system() == 'Darwin':  # Check if the OS is macOS
            # Here you can set a default for macOS or check for available GPUs if needed
            args['gpu_ids'] = ['0']  # Assume GPU is available; you may want to check explicitly
        else:
            # For other OS, use CUDA
            args['gpu_ids'] = os.environ.get('CUDA_VISIBLE_DEVICES', '').split(',')
    else:
        # Set the usable GPU as per the user input
        os.environ['USABLE_GPU'] = args['usable_gpu']
        args['gpu_ids'] = args['usable_gpu'].split(',')

    # Check if we have any usable GPUs
    assert 0 < len(args['gpu_ids']), 'Only GPU environments are supported'
